...
use uuid::Uuid;
use crate::consts;
use crate::utils::input::{TextualContent, validate_image};
...
pub async fn create_post(
    ...
    let mut uploaded_file_name = None;
    ...
    while let Some(field) = multipart.next_field().await? {
        ...
        if field_name == "text" {
            ...
            // Validate text content using TextualContent
            if let Some(valid_text) = TextualContent::try_new_short_form_content(&text) {
                text_content = Some(valid_text);
            } else {
                return Err((
            ...
            if let Err(err) = validate_image(&file_bytes, &filename) {
                return Err((
                    StatusCode::BAD_REQUEST,
                    format!("Invalid image: {}", err),
                )
                    .into());
            }

            // Generate a unique filename to avoid collisions and security issues
            ...
            uploaded_file_name = Some(unique_filename);
            uploaded_file_path = Some(format!("{}/{}", consts::UPLOADS_DIR, uploaded_file_name.as_ref().unwrap()));
        }
    }

    let text = text_content
        .ok_or((StatusCode::BAD_REQUEST, "Text content is required"))?
        .as_ref()
        .to_string();
